/*!
 * @nuxt/builder-edge v2.5.0-25853767.83db9595 (c) 2016-2019
...
class Builder {
  ...
  async resolveCustomTemplates(templateContext) {
    ...
    const templateFiles = await Promise.all(templateContext.templateFiles.map(async (file) => {
      ...
      const customPath = utilsEdge.r(this.options.srcDir, 'app', file);
      const customFileExists = await fsExtra.exists(customPath);

      return {
        src: customFileExists ? customPath : utilsEdge.r(this.template.dir, file),
        dst: file,
      ...
      .concat(
        this.options.build.templates.map((t) => {
          return Object.assign(
            {
              src: utilsEdge.r(this.options.srcDir, t.src || t),
  ...
  async compileTemplates(templateContext) {
    ...
    await Promise.all(
      templateFiles.map(async ({ src, dst, options, custom }) => {
        ...
        const _path = utilsEdge.r(this.options.buildDir, dst);
  ...
  watchClient() {
    ...
    const rGlob = dir => ['*', '**/*'].map(glob => utilsEdge.r(src, `${dir}/${glob}.{${this.supportedExtensions.join(',')}}`));

    let patterns = [
      utilsEdge.r(src, this.options.dir.layouts),
      utilsEdge.r(src, this.options.dir.store),
      utilsEdge.r(src, this.options.dir.middleware),
      ...rGlob(this.options.dir.layouts)
    ];

    if (this._nuxtPages) {
      patterns.push(
        utilsEdge.r(src, this.options.dir.pages),
  ...
  watchRestart() {
    const nuxtRestartWatch = [
      // Server middleware
      ...this.options.serverMiddleware.filter(utilsEdge.isString),
      // Custom watchers
      ...this.options.watch
